...
vector<const Agent*> deskMove(vector<const Agent*> agents)
{
  ...
  for(int i=0;i <agents.size();i++)
  {
    if(agents[i]->uuid.compare(temp)==0)
    {
      ...
      //vector<const Agent*>::iterator itt;
      //for(itt=agents.begin();itt!=agents.end();itt++)
      //{
      //  if((*itt)->location.compare(toDesk) ==0)
      //  {
      //    if((*itt)->name !="VACANT") cout << "Seat not empty!\nPlease choose another location!\n"; 
      //    else
      //    {
      //        cout << "Seat is empty!\nConfirm you want to move "
      //          << (*it)->name << " from " << (*it)->location << " to " 
      //          << (*itt)->location << " [y/n] ? : ";
      //        char yayNay;
      //        cin >> yayNay;
      //        cin.ignore(1000,10);
      //        if(yayNay =='y')
      //        {
      //          //note add operator = and copy constructor t pull this off
      //          Agent* temp = new Agent((*it)->uuid,(*it)->name, (*it)->om,toDesk,(*it)->desktop,(*it)->monitor);
      //    agents.push_back(temp);
      //    agents.erase(*itt);
      //        }
